...
- `unique_only` (bool, optional): If `True`, only emit the first occurrence of each `query_name`; subsequent segments with the same name are skipped. Default: `False`.
- `min_query_length` (int, optional): Discard reads whose sequence is shorter than this value (bp). Default: no minimum.
- `max_query_length` (int, optional): Discard reads whose sequence is longer than this value (bp). Default: no maximum.
